...
State: server **4.1.0**, **962 tests green** (23.08.2026), running natively on Windows. Branch `main` (multi-instrument merged; 23.08 gold-path audit: EA `InpExtremeSpreadPips`, real-risk echo `risk_usd`, broker-offset echo, per-instrument event policy/zones/stats gates — see `../wiki/pages/multi-instrument.md` and `research/GOLD_STRATEGY_LAB.md`). Docs wiki: `../wiki/index.md`.
...
│   ├── instrument_profiles.py    # Non-forex CFD units/clamps/event policy/zone+stats gates (XAUUSD 1 pip=$0.10, SL 60-120; GER40/US500 points); forex = None
...
│   ├── tools/                    # OFFLINE tools (build_learned_stats, strategy_lab, fetch_ff_calendar,
...
├── tests/                        # 918 tests (18.08.2026): unit/ integration/ e2e/ (pytest)
...
- **Event filter**: impact threshold (`MIN_IMPACT_LEVEL`) + name whitelist (TIER1/TIER2 + extras), or every event ≥ MIN_IMPACT when `TRADE_ALL_EVENTS` is ON. When a currency is ROUTED to a profiled instrument (`INSTRUMENT_ROUTING`), that profile's `extra_events` are added and `skip_events` removed for that currency (`config.routed_event_policy`; gold: + Core PCE / PPI, − New/Existing Home Sales — ranked with `tools/strategy_lab.py`). Speeches/testimony/press conferences (`NON_DATA_EVENT_MARKERS`) are NEVER traded in any mode. Shared predicate: `CalendarAggregator._event_is_tradeable`. The panel persists the **DISABLED complement** (`disabled_events`), so a roster name added by a newer server is enabled by default and the dashboard renders its checkboxes from `GET /api/config/events` (`tier1_events_all`/`tier2_events_all`) — never from a list baked into the HTML. Storing the ENABLED list instead is what silently un-traded FOMC/BoE/BoC for three weeks (18.08.2026 fix; legacy files are migrated on load via `LEGACY_PANEL_EVENT_ROSTER`). A Save declares the `roster` it rendered and the server only flips names inside it; a POST with **no** `roster` (the pre-fix panel, a script) is scoped to `LEGACY_PANEL_EVENT_ROSTER` and logs a warning, so a stale tab cannot re-disable the newer names *permanently* under the new key. `"roster": "*"` (`cfg.ROSTER_ALL`) opts into the full complement on purpose.
- **One decision per release**: events sharing currency + minute are ONE price path, so `event_cluster.py` picks a dominant (impact → family → modified-variant → name — the SAME order `tools/build_learned_stats.py` uses to bundle statistics; families match the long forms too (`Consumer Price Index`, `Gross Domestic Product`) and the BoC median/common variants count as modified, so a headline cannot lose its own release to a sibling) and the updater analyzes it once, marking the siblings analyzed and passing them into the prompt as `CO-RELEASED AT THE SAME MINUTE`. Serial analysis previously bought up to 4 panels per release, each against a shrinking panel deadline, and labelled the trade with the weakest sibling.
...
- **Lot sizing (EA)**: from SL distance; budget = min(balance × `InpRiskPercent`%, `max_loss_usd`), then a free-margin cap (`InpMaxMarginUsePercent`; 0 = auto = exactly the free margin, i.e. only a lot the broker would reject with 10019 is reduced — a forex lot the broker would fill is untouched). Optional lot reductions (`InpUseConfidenceLot`, `InpUseSpreadLotReduction` — default false since 23.08). Spread ENTRY blocks always active (`InpMaxSpreadPips`; ≥ `InpExtremeSpreadPips` never enters — 15 on forex = the old hard-coded value, 30 on XAUUSD; the 3/6/10/15 tier table scales with it). **The EA echoes `risk_usd` (lots × loss at the initial stop) + `margin_capped`** in every report: on a margin-capped gold chart the money at the stop is a fraction of the panel budget, and the server arms profit-protection / briefs the exit model / scales the rule-based fallback on `OpenPosition.effective_risk_usd()` (= the echo only when it is below the budget AND the trade is structurally capped — `margin_capped` or a profiled symbol; a forex trade sized at 70% via lot_percent keeps the budget, so forex numbers are unchanged). Deliberate forex-visible changes of 23.08: the rule-based exit fallback thresholds are now 30/60/40/−20/15% of the budget (identical at $100; $300/$600/$400/−$200/$150 at the production $1 000 instead of spread-noise dollars), and the exit prompt carries the entry panel's planned horizon (`planned_exit_minutes`). The EA also echoes `broker_utc_offset_sec` (authoritative over the bar-age inference) and the server marks a chart whose M1 bars stopped advancing as NOT fresh (`bars_advanced_at`).
...
**Primary: LLM panel** (reasoning-FIRST response schema; numeric fields clamped: conf 0–1, lot ≤ 85, exit 5–15 min, SL 25–80, TP 8–120 pips — the TP floor is low on purpose: the prompt anchors TP on the measured **favorable-run p80** for the event, and many tradeable events never travel 30 pips). Prompt context includes: COT, contrarian sentiment, forecast vs previous, M1 tail (20 candles) + cross-pair picture, EVENT PLAYBOOK (`knowledge/event_playbooks.json`), learned stats, calibration line (n≥50 **for the current model + prompt_version only** — it speaks in the second person, so it is never computed from another configuration's history), currency regimes, liquidity-pool stop clusters, RECENT TRADE OUTCOMES (realized P/L; `forced` and FAKE TEST rows filtered out).
...
python\venv\Scripts\python.exe -m pytest -q   # run tests (962, ~18 s, all green; SKYTOWER_OVERRIDES_FILE keeps them off the operator's panel state)
python\venv\Scripts\python.exe tools\strategy_lab.py --pair XAUUSD --impact HIGH   # offline strategy lab on recorded paths
```
