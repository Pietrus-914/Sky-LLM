...
| COT | CFTC | **Config bug, not a flaky source**: `CURRENCY_CONTRACTS` pins USD to `U.S. DOLLAR INDEX` and NZD to `NEW ZEALAND DOLLAR`; both series were retired in Feb 2022, so those two currencies have returned "Could not fetch COT data" for the project's entire life while CAD/GBP/AUD/EUR/JPY/CHF work. A lookup miss is reported as a fetch failure (`cot_analyzer.py:111`) |
